...
export default function Winners() {
	...
	const downloadCallers = () => {
		dispatch(getAllCallers(selectedDate, "progress"));
	};
	...
	return (
		<div>
			<p className={classes.header}>All Callers</p>
					...
					<Container maxWidth='sm'>
						...
						>
							{data.getAllCallersByDate.length > 0 ? (
								...
								>
									<CSVLink
										data={data.getAllCallersByDate}
										filename={"AllCallersList - " + selectedDate + ".csv"}
									>
								...
								<Grid
									...
									style={{
										background: data.progress ? "#039af4" : "#039af47d",
									}}
								>
									{data.progress ? (
										<CircularProgress
			...
			<Grid container justify='center' alignItems='center'>
				<Grid item xs={10} md={5} lg={4}>
					<Transition
						visible={data.noAllCallersByDate}
						animation='scale'
						duration={500}
					>
